...
export async function init(container) {
    ...
    // MỚI: THIẾT LẬP ÂM THANH (ĐÃ SỬA ĐỔI ĐỂ TỰ ĐỘNG PHÁT)
    ...
    audioLoader.load('../meditation.m4a', function(buffer) {
        ...
        // Cố gắng tự động phát ngay sau khi tải xong
        try {
            backgroundMusic.play();
        } catch (e) {
            console.warn("Browser prevented background music from autoplaying.", e);
        }
    });
    ...
    audioLoader.load('../bubbles.m4a', function(buffer) {
        ...
        try {
            ...
            console.warn("Browser prevented water sound from autoplaying.", e);
        }
    });
   
    // --- Chuẩn bị Vật liệu ---
    const textureLoader = new THREE.TextureLoader();
    const darkTileTexture = textureLoader.load('../stone_wall.jpeg');
    ...
    const ceiling = new THREE.Mesh(
        new THREE.BoxGeometry(roomWidth, ceilingThickness, roomLength),
        materials.ceiling
    );
